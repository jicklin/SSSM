...
define('tDesktop', function (require, exports, module) {
    ...
    var TDesktop = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            var weather = new TodayObj.Today.Weather({
                tDesktop: this
            });
...
define('tDesktop/tDesktop.IM', function (require, exports, module) {
    ...
    var Chat = {
        init: function (c) {
            ...
            self.uploader.on('fileQueued', function (file) {
                ...
                $list.append('<div class="chatmsg to clearfix" sms_id="" type_id="" url="" from_type_name="">' + '<div class="chat-popbox-dialogbox-time">' + time + '</div>' + '<div class="chat-popbox-dialogbox">' + '<div class="chat-popbox-avatar">' + '<img src="' + avatar + '" class="ui-li-thumb"/>' + '</div>' + '<div class="chat-popbox-bg">' + '<div id="' + file.id + '" class="item" title="' + file.name + '">' + '<div><input type="hidden" class="fileid" /><span class="filename">' + file.name + '</span><span class="filesize">( ' + filesize + ' )</span></div><div class="uploadimg"></div><p class="status"></p>' + '</div>' + '</div>' + '</div>' + '</div>');
                var $li = $('#' + file.id);
                ...
                newSmsArray[newSmsArray.length] = {
                    sms_id: "",
                    avatar: $("#loginavatar").attr("src"),
                    to_id: $(".chatuser.active").attr("user"),
                    from_id: loginUser.uid,
                    from_name: loginUser.user_name,
                    type_id: "0",
                    type_name: "ÍøÒ³°æÎ¢Ñ¶",
            ...
            self.uploader.on('uploadProgress', function (file, percentage) {
                ...
                if (!$percent.length) {
                    $percent = $('<div class="progress progress-striped active">' + '<div class="bar" role="progressbar" style="width: 0%">' + '</div>' + '</div>').appendTo($li).find('.bar');
                }
            ...
            $.ajax({
                ...
                dataType: 'text',
                success: function (data) {}, error: function (request, textStatus, errorThrown) {
            ...
            $.ajax({
                ...
                success: function (d) {
                    ...
                    $('#smsbox_msg_container').animate({
                        scrollTop: $('#smsbox_msg_container')[0].scrollHeight
                    }, 300);
                    newSmsArray[newSmsArray.length] = {
                        sms_id: d.sms_id,
                        avatar: data.avatar,
            ...
            itemsPerPage: 6,
            $el: $('#moreHistoryMsg'),
            init: function () {}, loadMore: function (currentUid) {
                var self = this;
                var uid = currentUid;
    ...
    var MultiSend = {
        init: function (c) {
            ...
            var self = this,
                $list = $('#uploadlist');
            $list.append('<div id="' + file.id + '" class="item" title="' + file.name + '">' + '<span class="filename"></span><img src="" /><input type="hidden" class="fileid" /><div class="close"></div>' + '</div>');
            var $li = $('#' + file.id);
        ...
        initialize: function (c) {
            ...
            $("#msg-tool .btn").click(function () {
                ...
                if (target == "nocbox" && tDesktop.isTouchDevice()) {
                    tDesktop.EventManager.trigger('iscroller:refresh', 'new_noc_list_wrapper');
                }
            });
        }
    });
...
define('tDesktop/tDesktop.Notification', function (require, exports, module) {
    ...
    function bindEvents(tD) {
        tD.EventManager.on('alert', function (arg) {
        ...
        tD.EventManager.on('message:read', function (arg) {
            var sms_ids = [];
            var smsIdStr = '';
            $.each(arg.items, function (index, item) {
                sms_ids.push(item.key);
...
define('tDesktop/tDesktop.Search', function (require, exports, module) {
    ...
    searchModule.initBind = function () {
        ...
        $(".search-container").delegate(".user-body-follow", "click", function () {
            ...
            $.get("/general/person_info/concern_user/concern_function.php", {
                load: "concern",
    ...
    function search(keyword, searchType, curPage, pageLimit) {
        ...
        if (searchType == 'menu') {
            ...
            for (var menuId in func_array) {
                ...
                if (menuId.substr(0, 1) != 'f' || func[1].substr(0, 1) == '@') {
                    continue;
                }
                var title = func[0] || "",
                    abbr = func[3] || "";
            ...
            success: function (data) {
                if (!self.paginators[searchType]) {
                    if (data.curPage > 0 && data.totalpage > 1) {
                        ...
                        self.paginators[searchType] = $('#pagination-' + searchType).data('bootstrapPaginator');
                    }
                }
                self.$el.animate({
                    height: '85%'
...
define('tDesktop/tDesktop.Tabs', function (require, exports, module) {
    ...
    var Tabs = {
        init: function () {
            ...
            //url = '/general/' + code;
                url = code; if (url.indexOf(".") < 0 && url.indexOf("?") < 0 && url.indexOf("#") < 0 && url.substring(url.length - 1) != "/")
                url += "/";
...
define('tDesktop/tDesktop.Today', function (require, exports, module) {
    ...
    var Deskdate = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            $().addTab('dt_date', td_lang.inc.msg_25, "/module/calendar2/", true);
        }, openTime: function () {
            $().addTab('dt_time_area', td_lang.inc.msg_26, "/module/world_time/", true);
        }
    });
